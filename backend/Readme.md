...
After importing all the routes & error MW, check if env is prod, if yes then use the prebuild frontend. but for dev it is better to run both individually for ease in development.

******************* error/errorHandler/catchAsyncErrors.js *******************
catchAsyncErrors: It is wrapper use to wrap function, It'll prevent app from crashing with unhandled rejections in function. {Unexpected}
...
NotFoundMW: We will use it to capture all api related 4xx error.
error: Global error handler to capture 5xx error, and we will send these error to frontend differently depends on dev/prod env. initialise it on app.js file.

**************************** Logic in Controller/route. ****************************
In desc. define purpose, method and whole route for clarity of purpose of controller
We are catching async error in "catchAsyncError.js" MW, which will catch error of our function. we will used "errorHandler.js" to send expected error to frontend, just need to provide them custom error message & status code. And finally global MW which will capture All the errors and send it to frontend as per our config.
We are defining multiple common Ops method in the model itself to avoid repition of code.
We are using isAuthenticator/isAuthorize to check if user loggedIn or allowed?
We are using jwtToken.js file to create jwtToken and store it in cookie & that cookie will save us from XSS and Csrf Attack. Storing token in browser instead of DB.
We are uploading/deleting images from cloudinary whenever needed/not. and storing the public_url & secure_url of those images in DB.
We used crypto to generate token and nodemailer to sendEmail. Used for Forget password recovery.
Pagination: We have created class ApiFeatures for pagination, which will take DB query & url query, and then do the needful operation & provide multiple filters.
In admin we still need to implement proper pagination, maybe not this much advanced.
server.js some very important set-up, needs to be implemented in every project.
